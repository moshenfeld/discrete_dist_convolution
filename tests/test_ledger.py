...
def test_ledger_with_ambiguous_cross_routing():
    ...
    add_neg, add_pos = infinity_ledger_from_pmfs(mX,pnegX,pposX,mY,pnegY,pposY,"DOMINATES")
    assert abs(add_neg - 0.12) < 1e-15     # unchanged
    assert abs(add_pos - (0.2 + 0.12)) < 1e-15  # cross to +∞
    add_neg2, add_pos2 = infinity_ledger_from_pmfs(mX,pnegX,pposX,mY,pnegY,pposY,"IS_DOMINATED")
